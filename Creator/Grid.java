...
//	public boolean isLinearlySolvable(int[][] g) {
//		int[][][] possibleNums = new int[9][9][9];
//		for (int i = 0; i < 9; i++) {
//			for (int j = 0; j < 9; j++) {
//				if (g[i][j] != 0) {
//					possibleNums[i][j][0] = 100;
//					possibleNums[i][j][1] = g[i][j];
//				} else {
//					for (int k = 0; k < 9; k++) {
//						possibleNums[i][j][k] = k;
//					}
//				}
//			}
//		}
...
//	}
	...
	public void initRandom(boolean easyMode) {
		// System.out.println("");
		System.out.println("Creating new sudoku");
		...
		while (count < limit) {
			...
			while (!correct) {
				...
				attempt++;
				if (attempt > 1000000) {
					System.out.println("Grid initiation failure, retrying..");
					attempt = 0;
					count = 0;
					for (int i = 0; i < 9; i++) {
						for (int j = 0; j < 9; j++) {
			...
			count++;
			if (count == limit && easyMode == true && !isEasy(newG)) {
				count = 0;
				for (int i = 0; i < 9; i++) {
					for (int j = 0; j < 9; j++) {
						newG[i][j] = 0;
		...
		// int[][] testG = { { 0, 5, 0, 0, 0, 0, 9, 4, 0 }, { 0, 0, 2, 4, 0, 0,
			...
			for (int j = 0; j < 9; j++) {
				if (this.grid[i][j] == 0) {
					...
					if (n[npos] == 10) {
						...
						tooHigh[0][1] = npos;
						// show(slottedGrid);
						// System.out.println("too hight" + " - " + i + " " + j
						// + " " + npos);
						return tooHigh;
					}
